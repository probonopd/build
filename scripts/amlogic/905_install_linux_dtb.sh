...
echo "Start copy system for LINUX partition."
...
if [ ! `mount $PART_ROOT $DIR_INSTALL` ] ; then
    echo "Not device $PART_ROOT.  STOP install !!!"
    return
fi
...
/usr/sbin/fw_setenv initargs "root=/dev/linux rootflags=data=writeback rw console=ttyS0,115200n8 console=tty0 no_console_suspend consoleblank=0 fsck.repair=yes net.ifnames=0 mac=\${mac}"

echo "*******************************************"
echo "Complete copy OS to eMMC parted LINUX"
echo "*******************************************"
